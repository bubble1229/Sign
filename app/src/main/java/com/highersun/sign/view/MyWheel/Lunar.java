...
public class Lunar
{
    ...
    //农历获取从1900年1月1日到指定日期的天数
    public static int getDayOf1900(int year,int month,int day,boolean isRun){
        int num=0;
        for (int i=0;i<year-1900;i++){
            num+=yearDays(1900+i);
        }
        for (int i=0;i<month-1;i++){
            num+=monthDays(year,1+i);
        }
        if(isRun||month>leapMonth(year)){
            num+=leapDays(year);
        }
        num+=day-1;
        return num;
    ...
    {// 返回阿拉伯数字的阴历日期
        ...
        return year+temp_mon + temp_day+leap;
    }
